...
# Region definitions (corrected to match original specification)
REGION_MAPPING = {
...
# Filter to 50 US states (exclude territories only)
us_states = states_gdf[~states_gdf['STUSPS'].isin(['PR', 'VI', 'GU', 'MP', 'AS'])]
print(f"Loaded {len(us_states)} US states (including AK, HI)")
...
state_names_list = [
    us_states[us_states['STUSPS'] == abbr]['NAME'].values[0]
